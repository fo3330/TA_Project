...
- [x] 포트폴리오용 프로젝트 주제 선정 (AI 워크로드를 Cloud 위에 설계·배포하는 형태)
  - 완료내용: "홈랩(Colima+k3d) → Terraform으로 클라우드 확장 → K8s 오케스트레이션 → GPU 노드에서 소형 오픈소스 LLM 서빙 → Prometheus/Grafana 모니터링"을 주제로 확정하고, ai_3 홈랩 프로젝트에서 실제로 구현까지 완료함.
- [x] 아키텍처 설계 문서(ADR, 시스템 다이어그램 등) 작성 연습
  - 완료내용: ADR 3건(Colima+k3d 선택 근거, NFS 트러블슈팅, LocalStack 대체 결정)과 통합 아키텍처 다이어그램을 작성 완료함.
...
- [x] 현재 업무 프로젝트의 AI 활용 경험을 포트폴리오 스토리로 재구성
  - 완료내용: KORUS 프로젝트의 AI 활용 경험(DB 마이그레이션 트러블슈팅, 애플리케이션 개발 보조, 포트폴리오 작성)을 포트폴리오 스토리로 재구성해 docs/current-project-ai-usage.md에 작성 완료함.
...
- [ ] 자격증 취득 진행 (해당 시)
  - CKA, AWS SAA(또는 AZ-305)를 목표로 설정했으나(ai_3 ROADMAP.md 기준) 아직 응시 전 — 남은 과제
