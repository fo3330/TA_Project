...
- [x] 학습 우선 벤더 선정 (AWS / GCP / Azure 중 1개, 근거는 [docs/role-definition.md](../docs/role-definition.md)에 기록)
  - 완료내용: AWS를 우선 학습 벤더로 결정함. ai_3 홈랩 프로젝트에서 LocalStack으로 VPC·EC2·S3를 실습했고, 목표 자격증도 AWS SAA로 설정해둔 상태.
...
- [x] 컴퓨트(VM/컨테이너/서버리스) 핵심 서비스 학습
  - 완료내용: Colima와 Docker로 컨테이너를 직접 실습함 — nginx 컨테이너에 볼륨을 마운트해 데이터 영속성을 구현했고, Docker Compose로 웹서버+DB 구조를 재현함. EC2는 LocalStack으로 개념 수준까지 실습.
...
- [x] 네트워크(VPC, 서브넷, 로드밸런서, DNS) 핵심 개념 학습
  - 완료내용: LocalStack으로 VPC를 실습하고, k3d 클러스터에 Calico CNI를 설치해 NetworkPolicy로 실제 트래픽을 차단하는 것까지 검증함.
...
- [x] 스토리지(오브젝트/블록/파일) 및 데이터베이스 서비스 개요 학습
  - 완료내용: S3는 LocalStack으로 실습했고, MinIO는 실제 오브젝트 스토리지로 구축(버킷 생성·업로드·조회까지 실증)했으며, local-path StorageClass 기반 PV/PVC 동적 프로비저닝까지 실습함.
...
- [x] IaC 개념 및 Terraform 등 도구 하나 맛보기 실습
  - 완료내용: Terraform으로 기존 Docker Compose 스택을 코드화했고, LocalStack을 대상으로 VPC·EC2·S3를 생성·삭제하는 것까지 실습을 완료함.
...
- [ ] 선택한 벤더의 입문 자격증 취득 여부 결정 및 진행
  - AWS SAA로 방향은 정했으나(ai_3 ROADMAP.md 기준) 아직 응시 전 — 남은 과제
